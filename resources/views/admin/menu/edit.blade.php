        ...
        <div class="card shadow border-0 rounded-4">
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="ingredients" class="form-control" rows="5" placeholder="Contoh: Roti Burger Daging Sapi Keju Cheddar Selada Tomat Mayonaise">{{ old('ingredients', $menu->ingredients) }}</textarea>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <label>Stok</label>
                        <input type="number" name="stock" class="form-control" value="{{ old('stock', $menu->stock) }}">
                    </div>
                </div>
                <div class="row mt-3">
                    <div class="col-md-6">
                        <label>Waktu Pembuatan (Menit)</label>
                        <input type="number" name="preparation_time" class="form-control" value="{{ old('preparation_time', $menu->preparation_time) }}">
                    </div>
                    <div class="col-md-6">
                        <label>Rating</label>
                        <input type="number" name="rating" min="1" max="5" step="0.1" class="form-control" value="{{ old('rating', $menu->rating) }}">
                    </div>
                    <div class="row mt-3">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            <label>Alergen</label>
                            <input type="text" name="allergen" class="form-control" placeholder="Contoh : Gluten, Susu, Kacang" value="{{ old('allergen',$menu->allergen) }}">
                        </div>
                    </div>
                </div>
                <div class="mt-3">
                    ...
                    @if($menu->image)
                        <div class="mb-2">
                            <img src="{{ asset('images/'.$menu->image) }}" width="120" class="rounded border">
                        </div>
                    @endif
                ...
                <div id="option-wrapper">
                    <button type="button" id="add-option" class="btn btn-success mt-3">+ Tambah Pilihan</button>
                    @foreach($menu->options as $i=>$option)
                        <div class="card p-3 mt-3 option-item">
                            <div class="mb-3">
                                <label>Nama Pilihan</label>
                                <input type="text" class="form-control" name="options[{{ $i }}][name]" value="{{ $option->name }}">
                            </div>
                            <div class="values">
                                @foreach($option->values as $j=>$value)
                                    <<div class="row mt-2">
                                        ...
                                        <div class="col-md-5">
                                            <input type="number" class="form-control" name="options[{{ $i }}][values][{{ $j }}][price]" value="{{ $value->extra_price }}">
                                        </div>
                                        <div class="col-md-2">
                            ...
                            <button type="button" class="btn btn-primary btn-sm mt-2 add-value">+ Tambah Nilai</button>
                            <button type="button" class="btn btn-danger btn-sm mt-2 remove-option">Hapus Pilihan</button>
                        </div>
                    @endforeach
                </div>
            </div>
...
document.addEventListener('click',function(e){
    ...
    if(e.target.classList.contains('add-value')){
        ...
        values.insertAdjacentHTML('beforeend',`
            <div class="row mt-2">
                <div class="col-md-5">
                    <input type="text" class="form-control" name="options[${index}][values][${total}][value]">
                </div>
                <div class="col-md-5">
                    <input type="number" class="form-control" value="0" name="options[${index}][values][${total}][price]">
                </div>
                <div class="col-md-2">
                    <button type="button" class="btn btn-danger remove-value">X</button>
                </div>
