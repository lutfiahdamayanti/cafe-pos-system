                                ...
                                <div class="card-body d-flex flex-column">
                                    <div class="pos-menu-content">
                                        ...
                                        <!-- <p class="text-success fw-bold mb-2">
                                            ...
                                            <br>
                                            Large : Rp {{ number_format($menu->price + $menu->large_price,0,',','.') }}
                                        </p> -->
                                            ...
                                            @foreach($menu->options as $option)
                                                ...
                                                <select class="form-select option-select mb-2">
                                                    ...
                                                    @foreach($option->values as $value)
                                                        <option value="{{ $value->id }}" data-name="{{ $option->name }}" data-value="{{ $value->value }}" data-price="{{ $value->extra_price }}">
                                                            {{ $value->value }}
                                                            @if($value->extra_price>0)
                                                                (+Rp {{ number_format($value->extra_price,0,',','.') }})
                                                            @endif
                                        ...
                                        <button type="button" class="btn btn-success w-100 mt-3 add-menu" data-id="{{ $menu->id }}" data-name="{{ $menu->name }}" data-price="{{ $menu->price }}">Tambahkan</button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
function renderCart(){
    ...
    cart.forEach((item,index)=>{
        ...
        <div class="border-bottom py-2">
            ...
            <br>Qty ${item.qty} x Rp ${item.price.toLocaleString()}
        </div>`;
        ...
        item.options.forEach(function(option,i){
            hidden+=`
            <input type="hidden" name="menus[${index}][options][]" value="${option.id}">
            `;
        });
    });

    if(cart.length==0) html=`<p class="text-muted">Belum ada menu.</p>`;

    let tax=subtotal*0.11,service=3000,total=subtotal+tax+service;
...
document.querySelectorAll('.add-menu').forEach(btn=>{
    btn.addEventListener('click',function(){
        let card=this.closest(".card-body"),size=card.querySelector(".size-select"),sizeValue="-";
        if(size) sizeValue=size.value;
        ...
        card.querySelectorAll(".option-select").forEach(function(select){
            if(select.value!=""){
                ...
                options.push({id:select.value,name:opt.dataset.name,value:opt.dataset.value});
            }
        });

        let note=card.querySelector(".note").value,id=this.dataset.id,name=this.dataset.name;
        let found=cart.find(item=>{
            return item.id==id&&item.size==sizeValue&&JSON.stringify(item.options)==JSON.stringify(options)&&item.note==note;
        });

        if(found) found.qty++;
        else cart.push({id:id,name:name,price:price,qty:1,size:sizeValue,options:options,note:note});
